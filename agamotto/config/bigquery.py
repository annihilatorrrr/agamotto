...
class BigQuery:
    def __init__(self, config):
        ...
        self._table_id = f"{self._gcp_project}.{self._gcp_dataset}.{self._gcp_bigquery_table}"
    ...
    def create_count_table(self):
        try:
            ...
            if self._bigquery_service.get_table(self._table_id):
                logger(self.__class__.__name__).info(f"Table {self._table_id} already exists")
        except NotFound:
            # CreateModel
    ...
    def insert_row(self, count):
        ...
        self._bigquery_service.query(f"INSERT INTO `{self._table_id}` VALUES ({count}, CURRENT_DATE(), CURRENT_TIME(), {self._location_latlong}, {self._location_id}, {self._location_name})")
    ...
    def insert(self, count_list: List[int]):
        try:
            ...
            for count, day_time in count_list:
                entry = AgamottoEntry(count=count, day_time=day_time, latlong=self._location_latlong, location_id=self._location_id, location_name=self._location_name).__dict__
                rows.append(entry)
            errors = self._get_bigquery_service().insert_rows_json(self._table_id, rows)  # Make an API request.
            if errors == []:
